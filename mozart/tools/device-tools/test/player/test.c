...
{
	printf("player controller uuid: %s, pos is %d, duration is %d.\n", snapshot->uuid, snapshot->position, snapshot->duration);

	switch (snapshot->status) {
	case PLAYER_PLAYING:
	case PLAYER_PAUSED:
	case PLAYER_UNKNOWN:
		printf("TODO: process player status: %s.\n", player_status_str[snapshot->status]);
		break;
	case PLAYER_STOPPED:
		printf("status: %s, play next music: %s.\n", player_status_str[snapshot->status], file);
