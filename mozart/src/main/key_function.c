...
extern int on_linein;
extern pthread_mutex_t linein_lock;
...
void mozart_wifi_mode(void)
{
	...
	if (infor.wifi_mode == STA || infor.wifi_mode == STANET || infor.wifi_mode == STA_WAIT){
		new_mode.cmd = SW_AP;
		memset(new_mode.param.switch_ap.ssid, 0, 32);
	}
	else if (infor.wifi_mode == AP || infor.wifi_mode == AP_WAIT){
		new_mode.cmd = SW_STA;
		memset(new_mode.param.switch_sta.ssid, 0, 32);
	}
...
void mozart_config_wifi(void)
{
	...
	new_mode.cmd = SW_AIRKISS;
	strcpy(new_mode.name, app_name);
	memset(new_mode.param.network_config.key, 0, 32);
	request_wifi_mode(new_mode);
}
...
void mozart_previous_song(void)
{
    ...
    case VR_DOMAIN:
        ...
        printf("TODO: vr speech pre music.\n");
...
#endif
    default:
        printf("Not in local play mode, do nothing in %s:%s:%d.\n",
               __FILE__, __func__, __LINE__);
        break;
    }
}
...
#elif (SUPPORT_VR == VR_SPEECH)
        printf("TODO: vr speech next music.\n");
#elif (SUPPORT_VR == VR_IFLYTEK)
...
#endif
    default:
        printf("Not in localplay mode or voice mode, do nothing in %s:%s:%d.\n",
               __FILE__, __func__, __LINE__);
        break;
    }
}

int mozart_stop_handler(bool mode_stop)
{
	int cnt = 0;
	int ret = -1;
	memory_domain domain;
...
void mozart_snd_source_switch(void)
{
	...
	//chech whethe the tf card is inserted
	// check if linein cable inserted.
	if (snd_source == SND_SRC_LINEIN) {
		if (!mozart_linein_is_in())
			snd_source = (snd_source + 1) % SND_SRC_MAX;
	}
	...
	case SND_SRC_BT_AVK:
		...
		/* Try to reconnect BT, if BT is not linked */
		if (!mozart_bluetooth_get_link_status()) {
			if (mozart_bluetooth_auto_reconnect(0) >= 0)
...
void mozart_music_favorite(void)
{
    memory_domain domain;
...
#if (SUPPORT_ATALK == 1)
    case ATALK_DOMAIN:
        mozart_atalk_love_audio();
	break;
#endif
    default:
	printf("%s. Music favorite is not valid\n", __func__);
	break;
    }
}
...
void mozart_volume_up(void)
{
    ...
    case BT_HS_DOMAIN:
	{
		...
		mozart_volume_set(hs_volume_set[i + 1], BT_VOLUME);
...
void mozart_music_list(int keyIndex)
{
	...
	case RENDER_DOMAIN:
		...
		if (strcmp(LAPSULE_PROVIDER, info->music_provider) == 0)
...
void mozart_factory_reset(void)
{
	mozart_stop();
#if (SUPPORT_ATALK == 1)
	mozart_stop_tone_sync();
	mozart_play_key_sync("atalk_factory_reset_36");
#endif
