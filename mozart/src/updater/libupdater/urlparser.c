...
url_parser *urlp_parse(url_parser *url, const char *path)
{
	...
	if (!sep1) {
		...
		if (sep1 > sep2) {
			...
			url->address = malloc(strlen(path_iter) - strlen(sep1) + 1);
			memset(url->address, 0, strlen(path_iter) - strlen(sep1) + 1);
			strncpy(url->address, path_iter, strlen(path_iter) - strlen(sep1));
...
int main(void)
{
	const char path[] = "http://10.4.50.227:11/release/version.ini";
	url_parser url;
