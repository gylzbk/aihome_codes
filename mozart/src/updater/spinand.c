...
int spinand_process_update(struct nv_info *info)
{
	int cnt = 0;
	int ret = 0;
	int nv_lock = -1;
	...
	if (info->update_method.method == UPDATE_TIMES) {
		pr_warn("spinand CANOT support UPDATE_TIMES update method right now!!\n");
		return -1;
	}
...
int spinand_prepare_new_version(struct nv_info *info)
{
	int cnt = 0;
	int ret = 0;
