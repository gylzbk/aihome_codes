...
#define QPLAY_PROVIDER "qplay"
	...
	typedef struct render_device_info {
		...
		/**
		 * @brief model name, default: mozart, Optional
		 */
		char *modelName;
		/**
	...
	/**
	 * @brief Render play prev song function.
	 *
	 * @return Success returned 0, others returned -1.
	 */
	extern int mozart_render_prev_song(void);

	/**
	 * @brief Render play next song function.
	 *
	 * @return Success returned 0, others returned -1.
	 */
	extern int mozart_render_next_song(void);
	 ...
	 */
	extern int mozart_render_action_callback(UpnpActionCallback AVTransportAction_callback,
